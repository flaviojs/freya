...
・Battle_Configに関する修正
	(battle.c)
・同一アカウントによるPT加入制限をしていてもPTに加入できて
　しまう問題を修正
	(party.c)
----------------------------------------
//2323 by ルーシェ
・mob_delay_rate_middle_boss、mob_delay_rate_mvp_bossが設定されていないとき、
　それらのMOBに対してmob_delay_rateが適用されていた不具合を修正。
	(src/map/npc.c)
----------------------------------------
//2322 by Quapla
...
(bin/tool/ladmin)
・パッチ評価スレッドpert2、>>33 名無し氏に指摘された箇所を修正
(db/packet_db.txt)
----------------------------------------
//2315 by 片山
...
・mmo.h内に説明文を追加
(src/common/mmo.h)
・homun.c内、腹減り時間の設定説明文を修正
(src/map/homun.c)
・パッチ評価スレッドpert2、>>32 free氏のpacket_dbを追加。
(db/packet_db.txt)
----------------------------------------
//2314 by Para

・sc_ison関数でサーバーが落ちるバグを修正
	（src/map/script.c）
・スクリプトに配列でカート内所持品を返すgetcartlist命令を追加
	（src/map/script.c）
・矢作成で装備している物が出てしまうバグを修正（thx CoCoa!）
	（src/map/clif.c）
・座る処理を少し変更
	（src/map/clif.c）
	（src/map/clif.h）
...
・ターボトラック実装
	（npc_quest_turbo.txt）
・ジュピロス保安区域実装
	（npc_quest_juperos.txt）
・商圏投票実装
	（npc_misc_zonda.txt）
・アリーナPTモード実装
　個人戦のモンスター位置や挙動を本鯖仕様化
	（npc_quest_arena.txt）
・飛行船スクリプトが巻き戻っていたのを修正
　襲撃イベントの挙動・確率などを本鯖仕様化
　ミニゲームのタイピングを本鯖仕様化
	（npc_town_airport.txt）
・アインブロック工場クエスト修正
　汚染イベントの発生条件、モンスター発生箇所を本鯖仕様化
	（npc_town_einbroch.txt）
・リヒタルゼン、治安維持イベントのバグを修正、本鯖仕様化
	（npc_town_lighthalzen.txt）
・ウンバラ夫婦喧嘩クエスト実装
...
・コモドの一部NPCの挙動を修正
	（npc_town_comodo.txt）
・初心者修練場、レベルに関係なく経験値を得られてしまうのを修正
　職業適性審査の判断箇所を本鯖仕様化
	（npc_job_00novice.txt）
・忍者の手甲作成クエスト実装
	（npc_job_29ninja.txt）
...
・忍者スキル「朔風」の攻撃範囲を修正
	（skill.c)
・ガンスリンガースキル「ピアーシングショット」をDEF無視攻撃に修正
	（battle.c）
...
・生命保険証の効果が無かったのを修正、
　item_dbのSCの設定でデスペナ軽減回数を設定可能に修正、
　例えば「sc_start SC_LIFEINSURANCE,1800000,3;」で3回デスペナを軽減します。
　(pc.c、status.c)
・武器精錬で精錬不可のものがリストに出てしまうのを修正
　BTS報告Cocoaさん thank's (clif.c)
・オシリスカード装備時にリザレクションをするとSP表示が追いついていなかったのを修正。
　BTS報告masterさん thank's (skill.c)

----------------------------------------
//2292 by Blaze

・ガンスリンガースキル「デスペラード」の修正
	（skill.c、skill_db.txt、skill_unit_db.txt)
・忍者スキル「火炎陣」のHIT計算がおかしかったのを修正
	（skill.c)
・マヤパープルカードの「タイガーアイ」効果が持続しなかったのを修正
	（status.c)
・実行範囲を持つNPCの範囲内でhideon→offをした時、スクリプトが再走しなかったのを修正
	（npc.c）
...
・陽光の箱、マヤパープルカード効果の「タイガーアイ」の修正
	（clif.c、status.c、status.h)
・NPCスキル「不死属性付与」効果時の挙動を修正
	（status.c、status.h)
・拳聖スキル「太陽と月と星の奇跡」を暫定実装
	（battle.c、battle.h)
・太陽と月と星の奇跡の発動確率を変更できるように修正
	（battle_athena.conf）
...
・NPCスキルの召喚系の仕様変更。種類 x Lv 匹 -> Lv 匹、召喚範囲の修正
	（mob.c)
・ソウルリンカースキル「エスク、エスウ、エスカ」の状態時、姿を維持しなかったのを修正
	（status.c)
・拳聖スキル「太陽と月と星の融合」状態のときは、移動速度が上がるように修正
	（status.c)
・拳聖スキル「太陽・月・星の温もり」のノックバック方向をMobの向きに依存するように修正
	（skill.c、skill_db.txt)
・MobスキルDBの召喚を変更に伴い修正
	（mob_skill_db.txt）
...
----------------------------------------
//2265-fix by ID:7QLFJSM

・VC で Windows のサービスとして動作するための機能を追加
　詳しくは doc/windows_service.txt を参照してください。
・基本となる構築方法がまったく書いてないのでドキュメント化。
　（doc/server_build.txt）

- fix : SQL でコンパイルできない問題をたぶん修正
- fix : Warning 4819 を無視するように変更

(doc/)
server_build.txt	サーバーの基本構築方法
windows_service.txt	Windows でサービス化するためのドキュメント
(bin/tool)
inst_winsvc.bat		サービスのインストール
uninst_winsvc.bat	サービスのアンインストール
(src/common)
atnwinsvc.*		新規追加。サービスの機能を提供
core.c			サービスの機能の呼び出しなど
Makefile		atnwinsvc.o の追加
(src/login)
login.c			サービスの機能の呼び出しなど
Makefile		atnwinsvc.o の追加
(src/char)
char.c			サービスの機能の呼び出しなど
Makefile		atnwinsvc.o の追加
(src/map)
map.c, npc.c	サービスの機能の呼び出しなど
script.c		型変換の警告つぶし
Makefile		atnwinsvc.o の追加
(/)
vc_make.bat		サービス関係の設定追加
bcc32_make.bat	atnwinsvc.c の追加
...
・2261のbattle_calc_weapon_attack() に関する変更を全て巻き戻す（battle.c）
	戦闘ダメージ関連の処理関連をこれ以上サポートする予定がなく、バグを含んだままの中途半端な状態で
	止まってしまうため元に戻しておきます
	確認できた範囲においては2261の変更によって矢のATKがダメージに加算されない可能性があるバグを
	発生させてしまっていました。
	重ね重ねお詫び申し上げます。
...
・アイテム合成の修正(skill.c, skill.h, clif.c)
・トワイライトファーマシーが成功個数/ランキングポイントを一度に獲得するように(skill.c)
・被弾共闘が入らないのを修正(pc.c)
・script function: arrayselect()を追加(script.c)
・arrayselect()の説明を追加(script_ref.txt)
・arrayselect()のサンプルを追加(npc_test_arrayselect.txt)
----------------------------------------
//2261 by Rayce
...
・status_get_opt[1-3] のバグ修正(status.c map.h homun.c thanks to (　・ω・)ﾓﾆｭ? さん) 
