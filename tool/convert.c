...
int mmo_char_tostr(char *str, struct mmo_charstatus *p)
{
	...
	for(i = 0; i < GLOBAL_REG_NUM; i++)
		if (p->global_reg[i].str[0] && p->global_reg[i].value != 0)
...
static inline int mmo_char_fromstr(char *str,struct mmo_charstatus *p) {
	int tmp_int[256];
	char tmp_str[256];
	int set, next, len, i;
	...
	while(str[next] && str[next] != '\t' && str[next] != '\n' && str[next] != '\r' && i < GLOBAL_REG_NUM) {
		...
		if (tmp_str[0] && tmp_int[0] != 0) {
			strncpy(p->global_reg[i].str, tmp_str, 32);
...
int mmo_char_convert(char *fname1,char *fname2)
{
	...
	while(fgets(line, sizeof(line), ifp)) { // fgets reads until maximum one less than size and add '\0' -> so, it's not necessary to add -1
