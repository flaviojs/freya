// $Id: atcommand.c 683 2006-07-08 02:48:38Z zugule $
#include <config.h>
...
ATCOMMAND_FUNC(jobchange) {
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1 || job < 0 || job >= MAX_PC_CLASS || (job > 26 && job < 4001)) {
		...
		if ((i == (int)(sizeof(jobs) / sizeof(jobs[0])))) {
			...
			send_usage(fd, "   1 Swordman          8 Priest           15 Monk             22 N/A");
			send_usage(fd, "   2 Mage              9 Wizard           16 Sage             23 Super Novice");
			send_usage(fd, "   3 Archer           10 Blacksmith       17 Rogue            24 Gunslinger");
			send_usage(fd, "   4 Acolyte          11 Hunter           18 Alchemist        25 Ninja");
			send_usage(fd, "   5 Merchant         12 Assassin         19 Bard             26 N/A");
			...
			send_usage(fd, "4026 Baby Archer    4033 Baby Blacksmith  4040 Baby Rogue     4047 Taekwon Master");
			send_usage(fd, "4027 Baby Acolyte   4034 Baby Hunter      4041 Baby Alchemist 4048 N/A");
