...
static int refinebonus[5][3]; // ê∏òBÉ{Å[ÉiÉXÉeÅ[ÉuÉã(refine_db.txt)
int percentrefinery[5][10]; // ê∏òBê¨å˜ó¶(refine_db.txt)
...
int status_percentrefinery(struct map_session_data *sd, struct item *item) {
	...
	percent += pc_checkskill(sd, BS_WEAPONRESEARCH); // ïêäÌå§ãÜÉXÉLÉãèäéù
...
int status_calc_pc(struct map_session_data* sd, int first) {
	...
	pc_calc_skilltree(sd); // ÉXÉLÉãÉcÉäÅ[ÇÃåvéZ
		...
		if(sd->inventory_data[idx]) {
			if(sd->inventory_data[idx]->type == 4) {
				if(sd->status.inventory[idx].card[0]!=0x00ff && sd->status.inventory[idx].card[0]!=0x00fe && sd->status.inventory[idx].card[0]!=(short)0xff00) {
					int j;
					for(j=0;j<sd->inventory_data[idx]->slot;j++){	// ÉJÅ[Éh
			...
			else if(sd->inventory_data[idx]->type==5){ // ñhãÔ
				if(sd->status.inventory[idx].card[0]!=0x00ff && sd->status.inventory[idx].card[0]!=0x00fe && sd->status.inventory[idx].card[0]!=(short)0xff00) {
					int j;
					for(j=0;j<sd->inventory_data[idx]->slot;j++){	// ÉJÅ[Éh
						int c=sd->status.inventory[idx].card[j];
...
//				run_script(sd->petDB->script, 0, sd->bl.id, 0);
	...
	// ëïîıïiÇ…ÇÊÇÈÉXÉeÅ[É^ÉXïœâªÇÕÇ±Ç±Ç≈é¿çs
	for(i=0;i<10;i++) {
		idx = sd->equip_index[i];
		if(idx < 0)
			continue;
		...
		if(sd->inventory_data[idx]) {
			...
			if(sd->inventory_data[idx]->type == 4) {
				...
				if(i == 8 && sd->status.inventory[idx].equip == 0x20) {
					...
					sd->watk_2 = (r=sd->status.inventory[idx].refine)*	// ê∏òBçUåÇóÕ
						refinebonus[wlv][0];
					if( (r-=refinebonus[wlv][2])>0 )	// âﬂèËê∏òBÉ{Å[ÉiÉX
						sd->overrefine_ = r*refinebonus[wlv][1];

					if(sd->status.inventory[idx].card[0] == 0x00ff) {	// êªë¢ïêäÌ
						sd->star_ = (sd->status.inventory[idx].card[1]>>8);	// êØÇÃÇ©ÇØÇÁ
						wele_ = (sd->status.inventory[idx].card[1]&0x0f);	// ëÆ ê´
					...
					sd->watk2 += (r = sd->status.inventory[idx].refine) *	// ê∏òBçUåÇóÕ
						refinebonus[wlv][0];
					if( (r-=refinebonus[wlv][2])>0 )	// âﬂèËê∏òBÉ{Å[ÉiÉX
						sd->overrefine += r*refinebonus[wlv][1];

					if(sd->status.inventory[idx].card[0] == 0x00ff) {	// êªë¢ïêäÌ
						sd->star += (sd->status.inventory[idx].card[1]>>8);	// êØÇÃÇ©ÇØÇÁ
						wele = (sd->status.inventory[idx].card[1]&0x0f);	// ëÆ ê´
						if(ranking_id2rank(*((unsigned long *)(&sd->status.inventory[idx].card[2])), RK_BLACKSMITH))
							sd->star += 10; //Forged weapons from the top10 blacksmith will receive an additional absolute damage of 10 (pierces vit/armor def) [Proximus]
					}
	...
	if(sd->equip_index[10] >= 0){ // ñÓ
		...
		if(sd->inventory_data[idx]){		//Ç‹ÇæëÆê´Ç™ì¸Ç¡ÇƒÇ¢Ç»Ç¢
			sd->state.lr_flag = 2;
			run_script(sd->inventory_data[idx]->equip_script,0,sd->bl.id,0);
			sd->state.lr_flag = 0;
	...
	if(sd->sc_count)
	{
		...
		if (sd->sc_data[SC_INCREASEAGI].timer != -1 && sd->sc_data[SC_QUAGMIRE].timer == -1 && sd->sc_data[SC_DONTFORGETME].timer == -1) { // ë¨ìxëùâ¡
		...
		if(sd->sc_data[SC_GLORIA].timer!=-1)
			sd->paramb[5]+= 30;
		if(sd->sc_data[SC_LOUD].timer!=-1 && sd->sc_data[SC_QUAGMIRE].timer == -1)	// ÉâÉEÉhÉ{ÉCÉX
			sd->paramb[0]+= 4;
		...
		if(sd->sc_data[SC_TRUESIGHT].timer!=-1){	// ÉgÉDÉãÅ[ÉTÉCÉg
			sd->paramb[0]+= 5;
			sd->paramb[1]+= 5;
			sd->paramb[2]+= 5;
			sd->paramb[3]+= 5;
...
//			sd->weapon_atk += sd->sc_data[SC_WATKFOOD].val1;
	...
	if((skill = pc_checkskill(sd, BS_WEAPONRESEARCH)) > 0)	// ïêäÌå§ãÜÇÃñΩíÜó¶ëùâ¡
		sd->hit += skill << 1;
	...
	if(sd->sc_count && sd->sc_data[SC_BERSERK].timer != -1) {	// ÉoÅ[ÉTÅ[ÉN
	...
	/*if((skill=pc_checkskill(sd,HP_MEDITATIO)) > 0) { // ÉÅÉfÉBÉeÉCÉeÉBÉIÇÕSPRÇ≈ÇÕÇ»Ç≠é©ëRâÒïúÇ…Ç©Ç©ÇÈ
		sd->nhealsp += 3 * skill * (sd->status.max_sp) / 1000; // fixed by [Yor]
		if (sd->nhealsp > 0x7fff) sd->nhealsp = 0x7fff;
	}*/

	// éÌë∞ëœê´ÅiÇ±ÇÍÇ≈Ç¢Ç¢ÇÃÅH ÉfÉBÉoÉCÉìÉvÉçÉeÉNÉVÉáÉìÇ∆ìØÇ∂èàóùÇ™Ç¢ÇÈÇ©Ç‡Åj
	if( (skill=pc_checkskill(sd,SA_DRAGONOLOGY))>0 ){ // ÉhÉâÉSÉmÉçÉWÅ[
		skill = skill*4;
		sd->addrace[9]+=skill;
		sd->addrace_[9]+=skill;
		sd->subrace[9]+=skill;
	...
	//Fleeè„è∏
	if ((skill = pc_checkskill(sd, TF_MISS)) > 0) {	// âÒîó¶ëùâ¡
	...
	// ÉXÉLÉãÇ‚ÉXÉeÅ[É^ÉXàŸèÌÇ…ÇÊÇÈécÇËÇÃÉpÉâÉÅÅ[É^ï‚ê≥
	if(sd->sc_count) {
		// ATK/DEFïœâªå`
		...
		if (sd->sc_data[SC_POISON].timer != -1)	// ì≈èÛë‘
		...
		if (sd->sc_data[SC_DRUMBATTLE].timer != -1){	// êÌëæå€ÇÃãøÇ´
		...
		if(sd->sc_data[SC_NIBELUNGEN].timer!=-1) {	// ÉjÅ[ÉxÉãÉìÉOÇÃéwó÷
		...
		if(sd->sc_data[SC_VOLCANO].timer!=-1 && sd->def_ele==3){	// É{ÉãÉPÅ[Ém
			sd->watk += sd->sc_data[SC_VIOLENTGALE].val3;
		}

		if(sd->sc_data[SC_SIGNUMCRUCIS].timer!=-1)
			sd->def = sd->def * (100 - sd->sc_data[SC_SIGNUMCRUCIS].val2)/100;
		if(sd->sc_data[SC_ETERNALCHAOS].timer!=-1) { // ÉGÉ^Å[ÉiÉãÉJÉIÉX
			sd->def = 0;
			sd->def2 = 0;
		}

		if(sd->sc_data[SC_CONCENTRATION].timer!=-1){ //ÉRÉìÉZÉìÉgÉåÅ[ÉVÉáÉì
		...
		if(sd->sc_data[SC_MAGICPOWER].timer!=-1){ //ñÇñ@óÕëùïù
		...
		if (sd->sc_data[i=SC_ASPDPOTION3].timer != -1 ||
		    ...
		    sd->sc_data[i=SC_ASPDPOTION0].timer != -1) // ëù ë¨É|Å[ÉVÉáÉì
			aspd_rate -= sd->sc_data[i].val2;
		if (sd->sc_data[SC_WINDWALK].timer != -1 && sd->sc_data[SC_INCREASEAGI].timer == -1) //ÉEÉBÉìÉhÉEÉH?ÉNéûÇÕLv*2%å∏éZ
		...
		if(sd->sc_data[SC_BLIND].timer!=-1){	// à√çï
			sd->hit -= sd->hit >> 2;
			sd->flee -= sd->flee >> 2;
		}
		if (sd->sc_data[SC_WINDWALK].timer != -1) // ÉEÉBÉìÉhÉEÉHÅ[ÉN
			sd->flee += sd->flee * (sd->sc_data[SC_WINDWALK].val2) / 100;
		if (sd->sc_data[SC_SPIDERWEB].timer != -1) //ÉXÉpÉCÉ_Å[ÉEÉFÉu
			sd->flee -= sd->flee * 50 / 100;
		if (sd->sc_data[SC_TRUESIGHT].timer != -1) //‚g‚D‚Ô¸[‚T‚C‚g
			sd->hit += sd->hit * 3 * (sd->sc_data[SC_TRUESIGHT].val1) / 100;
		if (sd->sc_data[SC_CONCENTRATION].timer != -1) //ÉRÉìÉZÉìÉgÉåÅ[ÉVÉáÉì
			sd->hit += 10 * sd->sc_data[SC_CONCENTRATION].val1;

		// ëœê´
		if(sd->sc_data[SC_SIEGFRIED].timer != -1){  // ïséÄêgÇÃÉWÅ[ÉNÉtÉäÅ[Éh
		...
		if(sd->sc_data[SC_PROVIDENCE].timer != -1){	// ÉvÉçÉîÉBÉfÉìÉX
			sd->subele[6] += sd->sc_data[SC_PROVIDENCE].val2;	// ëŒ êπëÆê´
			sd->subrace[6] += sd->sc_data[SC_PROVIDENCE].val2;	// ëŒ à´ñÇ
		}
		...
		if(sd->sc_data[SC_DELUGE].timer!=-1 && sd->def_ele==1){	// ÉfÉäÉÖÅ[ÉW
			sd->status.max_hp += sd->status.max_hp*sd->sc_data[SC_DELUGE].val3/100;
			if(sd->status.max_hp < 0 || sd->status.max_hp > battle_config.max_hp)
				sd->status.max_hp = battle_config.max_hp;
		}
		if(sd->sc_data[SC_SERVICEFORYOU].timer!=-1) {	// ÉTÅ[ÉrÉXÉtÉHÅ[ÉÜÅ[
		...
		if (sc_data) {
			...
			if (sc_data[SC_BLESSING].timer != -1) {
				...
				if (battle_check_undead(race, status_get_elem_type(bl)) || race == 6)
					...
					str += sc_data[SC_BLESSING].val1; // ÇªÇÃëº
			}
	...
	if (bl->type == BL_PC)
		...
		if (sc_data) {
			if (sc_data[SC_QUAGMIRE].timer != -1) // ÉNÉ@ÉOÉ}ÉCÉA
				...
				if (sc_data[SC_INCREASEAGI].timer != -1 && sc_data[SC_DONTFORGETME].timer == -1) // ë¨ìxëùâ¡(PCÇÕpc.cÇ≈)
		...
		if (sc_data) {
			...
			if (sc_data[SC_BLESSING].timer != -1) { // ÉuÉåÉbÉVÉìÉO
				...
				if (battle_check_undead(race, status_get_elem_type(bl)) || race == 6)
					...
					dex += sc_data[SC_BLESSING].val1; // ÇªÇÃëº
			}
			if (sc_data[SC_QUAGMIRE].timer != -1) // ÉNÉ@ÉOÉ}ÉCÉA
				dex -= sc_data[SC_QUAGMIRE].val1 * 10;
			if (sc_data[SC_TRUESIGHT].timer != -1) // ÉgÉDÉãÅ[ÉTÉCÉg
				dex += 5;
			if (sc_data[SC_INCDEX].timer != -1) // ÉgÉDÉãÅ[ÉTÉCÉg
	...
	if (bl->type == BL_PC)
		...
		if (sc_data) {
			...
			if (sc_data[SC_BLIND].timer != -1) // éÙÇ¢
				hit -= hit * 25 / 100;
			if (sc_data[SC_TRUESIGHT].timer != -1) // ‚g‚D‚Ô¸[‚T‚C‚g
	...
	if (bl->type == BL_PC) {
		...
		if (sc_data) { //èÛë‘àŸèÌÇ†ÇË
			if (sc_data[SC_PROVOKE].timer != -1) //PCÇ≈ÉvÉçÉ{ÉbÉN(SM_PROVOKE)èÛë‘
				batk = batk * (100 + (3 * sc_data[SC_PROVOKE].val1 + 2)) / 100; //base_atkëùâ¡  Formula correction. [Bison]
...
//		case SC_CONFUSION:
	...
	if(sd && !(flag&8)) {
		if(SC_STONE <= type && type <= SC_BLIND){	/* ÉJÅ[ÉhÇ…ÇÊÇÈëœê´ */
			if(sd->reseff[type - SC_STONE] > 0 && rand() % 10000 < sd->reseff[type - SC_STONE]){
				if(battle_config.battle_log)
					printf("PC %d skill_sc_start: cardÇ…ÇÊÇÈàŸèÌëœê´î≠ìÆ\n",sd->bl.id);
				return 0;
			}
		}
	}
	...
	/*case SC_UTSUSEMI:
		...
		case SC_PROVOKE:
			scflag.calc = 1;
			if (tick <= 0) tick = 1000;	/* (ÉIÅ[ÉgÉoÅ[ÉTÅ[ÉN) */
			break;
		case SC_ENDURE:
			if (tick <= 0)
			tick = 1000 * 60;
...
//				if(tick < 1000) tick = 1000;
				tick = 30000;//êáñ∞ÇÕÉXÉeÅ[É^ÉXëœê´Ç…ä÷ÇÌÇÁÇ∏30ïb
			}
			break;
		case SC_FREEZE:
			if(!(flag&2)) {
...
//		case SC_MARIONETTE:
		...
		case SC_REJECTSWORD:
			val2 = 3; //3âÒçUåÇÇíµÇÀï‘Ç∑
			val3 = 0; // Damage reflect state - [Aalye]
			break;

		case SC_MEMORIZE:
			val2 = 5; //3âÒârè•Ç1/5Ç…Ç∑ÇÈ // Memorize is supposed to reduce the cast time of the next 5 spells by half (thanks to [BLB] from freya's bug report)
			break;
...
#endif
		clif_status_change(bl, StatusIconTable[type], 1); /* ÉAÉCÉRÉìï\é¶ */
	}
	...
	switch(type){
		...
		case SC_STUN:
		case SC_SLEEP:
			battle_stopattack(bl);	/* çUåÇí‚é~ */
			skill_stop_dancing(bl,0);	/* ââët/É_ÉìÉXÇÃíÜíf */
			{	/* ìØéûÇ…ä|Ç©ÇÁÇ»Ç¢ÉXÉeÅ[É^ÉXàŸèÌÇâèú */
				int i;
	...
	if (scflag.send_opt) /* optionÇÃïœçX */
		clif_changeoption(bl);

	(*sc_count)++; /* ÉXÉeÅ[É^ÉXàŸèÌÇÃêî */
	...
	if(sd) {
		if(scflag.calc)
			status_calc_pc(sd, 0); /* ÉXÉeÅ[É^ÉXçƒåvéZ */
		if(type == SC_RUN)
			pc_run(sd, val1, val2);
	}
...
int status_change_end(struct block_list* bl, int type, int tid)
{
	...
	if ((*sc_count) > 0 && sc_data[type].timer != -1 && (sc_data[type].timer == tid || tid == -1)) {
		...
		/* äYìñÇÃàŸèÌÇê≥èÌÇ…ñﬂÇ∑ */
		sc_data[type].timer = -1;
		(*sc_count)--;

		switch(type) {	/* àŸèÌÇÃéÌóﬁÇ≤Ç∆ÇÃèàóù */
			...
			case SC_DELUGE:
			case SC_VIOLENTGALE:
			case SC_ETERNALCHAOS:		/* ÉGÉ^Å[ÉiÉãÉJÉIÉX */
			case SC_DRUMBATTLE:			/* êÌëæå€ÇÃãøÇ´ */
			case SC_NIBELUNGEN:			/* ÉjÅ[ÉxÉãÉìÉOÇÃéwó÷ */
			case SC_SIEGFRIED:			/* ïséÄêgÇÃÉWÅ[ÉNÉtÉäÅ[Éh */
			...
			case SC_SERVICEFORYOU:			/* ÉTÅ[ÉrÉXÉtÉHÅ[ÉÜÅ[ */
			case SC_EXPLOSIONSPIRITS:	// îöóÙîgìÆ
			case SC_STEELBODY:			// ã‡çÑ
			case SC_ASPDPOTION0:		/* ëùë¨É|Å[ÉVÉáÉì */
			case SC_ASPDPOTION1:
			case SC_ASPDPOTION2:
			case SC_ASPDPOTION3:
			...
			case SC_RIDING:
			case SC_BLADESTOP_WAIT:
			case SC_AURABLADE:			/* ÉIÅ[ÉâÉuÉåÅ[Éh */
			case SC_PARRYING:			/* ÉpÉäÉCÉìÉO */
			case SC_CONCENTRATION:		/* ÉRÉìÉZÉìÉgÉåÅ[ÉVÉáÉì */
			case SC_TENSIONRELAX:		/* ÉeÉìÉVÉáÉìÉäÉâÉbÉNÉX */
			case SC_ASSUMPTIO:			/* ÉAÉVÉÉÉìÉvÉeÉBÉI */
			case SC_WINDWALK:		/* ÉEÉCÉìÉhÉEÉHÅ[ÉN */
			case SC_TRUESIGHT:		/* ÉgÉDÉãÅ[ÉTÉCÉg */
			case SC_SPIDERWEB:		/* ÉXÉpÉCÉ_Å[ÉEÉFÉbÉu */
			case SC_MAGICPOWER:
			case SC_CHASEWALK:
			...
			case SC_AUTOBERSERK:
				...
				break;
			case SC_BERSERK:			/* ÉoÅ[ÉTÅ[ÉN */
				calc_flag = 1;
				break;
            case SC_DEVOTION:        /* ÉfÉBÉ{Å[ÉVÉáÉì */
                {
			...
			case SC_BLADESTOP:
				...
				//ï–ï˚Ç™êÿÇÍÇΩÇÃÇ≈ëäéËÇÃîíênèÛë‘Ç™êÿÇÍÇƒÇ»Ç¢ÇÃÇ»ÇÁâèú
			...
			case SC_NOCHAT:	//É`ÉÉÉbÉgã÷é~èÛë‘
				{
					struct map_session_data *sd = NULL;
					if (bl->type == BL_PC && (sd = (struct map_session_data *)bl)) {
			...
			case SC_SPLASHER:		/* ÉxÉiÉÄÉXÉvÉâÉbÉVÉÉÅ[ */
				{
					struct block_list *src=map_id2bl(sc_data[type].val3);
					if(src && tid!=-1){
						//é©ï™Ç…É_ÉÅÅ[ÉWÅïé¸àÕ3*3Ç…É_ÉÅÅ[ÉW
			...
			case SC_CLOSECONFINE2:	// target confine ended
				...
				break;
			case SC_SELFDESTRUCTION:		/* é©îö */
				{
					//é©ï™ÇÃÉ_ÉÅÅ[ÉWÇÕ0Ç…ÇµÇƒ
					struct mob_data *md = NULL;
		...
		if (bl->type == BL_PC)
			clif_status_change(bl, StatusIconTable[type], 0);	/* ÉAÉCÉRÉìè¡ãé */

		switch(type){	/* ê≥èÌÇ…ñﬂÇÈÇ∆Ç´Ç»Ç…Ç©èàóùÇ™ïKóv */
		case SC_STONE:
		case SC_FREEZE:
		case SC_STUN:
		case SC_SLEEP:
		...
		case SC_POISON:
			...
			*opt2 &= ~1;					// Ç™êÍópÇ…ópà”Ç≥ÇÍÇΩèÍçáÇ…ÇÕ
		...
		case SC_SIGHT:
			*option &= ~1;
			opt_flag = 1;
			break;
		case SC_WEDDING:	//åãç•óp(åãç•àﬂè÷Ç…Ç»Ç¡Çƒï‡Ç≠ÇÃÇ™íxÇ¢Ç∆Ç©)
			*option &= ~4096;
			opt_flag = 1;
			break;
		case SC_RUWACH:
		...
		case SC_CONCENTRATION:		/* ÉRÉìÉZÉìÉgÉåÅ[ÉVÉáÉì */
			*opt3 &= ~1;
			break;
		case SC_OVERTHRUST:			/* ÉIÅ[ÉoÅ[ÉXÉâÉXÉg */
			*opt3 &= ~2;
			break;
		case SC_ENERGYCOAT:			/* ÉGÉiÉWÅ[ÉRÅ[Ég */
			*opt3 &= ~4;
			break;
		case SC_EXPLOSIONSPIRITS:	// îöóÙîgìÆ
			*opt3 &= ~8;
		...
		case SC_MOONLIT:
			*opt3 &= ~512;
			break;
		case SC_MARIONETTE:		/* É}ÉäÉIÉlÉbÉgÉRÉìÉgÉçÅ[Éã */
		case SC_MARIONETTE2:
			*opt3 &= ~1024;
			break;
		...
		if(opt_flag)	/* optionÇÃïœçXÇì`Ç¶ÇÈ */
			clif_changeoption(bl);

		if (bl->type == BL_PC && calc_flag)
			status_calc_pc((struct map_session_data *)bl,0);	/* ÉXÉeÅ[É^ÉXçƒåvéZ */
	}
...
int status_change_timer(int tid, unsigned int tick, int id, int data)
{
	...
	nullpo_retr(0, bl = map_id2bl(id)); //äYìñIDÇ™Ç∑Ç≈Ç…è¡ñ≈ÇµÇƒÇ¢ÇÈÇ∆Ç¢Ç§ÇÃÇÕÇ¢Ç©Ç…Ç‡Ç†ÇËÇªÇ§Ç»ÇÃÇ≈ÉXÉãÅ[ÇµÇƒÇ›ÇÈ
	nullpo_retr(0, sc_data = status_get_sc_data(bl));
	...
	switch(type) { /* ì¡éÍÇ»èàóùÇ…Ç»ÇÈèÍçá */
	case SC_MAXIMIZEPOWER: /* É}ÉLÉVÉ}ÉCÉYÉpÉèÅ[ */
	case SC_CLOAKING: /* ÉNÉçÅ[ÉLÉìÉO */
		if (sd) {
			if (sd->status.sp > 0) {	/* SPêÿÇÍÇÈÇ‹Ç≈éùë± */
				sd->status.sp--;
				clif_updatestatus(sd, SP_SP);
				sc_data[type].timer = add_timer(sc_data[type].val2 + tick, status_change_timer, bl->id, data);/* É^ÉCÉ}Å[çƒê›íË */
				return 0;
			}
		}
		break;
	...
	case SC_HIDING: /* ÉnÉCÉfÉBÉìÉO */
		if (sd) { /* SPÇ™Ç†Ç¡ÇƒÅAéûä‘êßå¿ÇÃä‘ÇÕéùë± */
			if (sd->status.sp > 0 && (--sc_data[type].val2) > 0) {
				if(sc_data[type].val2 % (sc_data[type].val1 + 3) == 0) {
					sd->status.sp--;
					clif_updatestatus(sd, SP_SP);
				}
				sc_data[type].timer = add_timer(1000 + tick, status_change_timer, bl->id, data); /* É^ÉCÉ}Å[çƒê›íË */
				return 0;
			}
		}
		break;
	...
	case SC_SIGHTBLASTER:		
	  {
		...
		if ((--sc_data[type].val2) > 0) {
			sc_data[type].timer = add_timer(250 + tick, status_change_timer, bl->id, data); /* É^ÉCÉ}Å[çƒê›íË */
			return 0;
		}
	  }
		break;
	...
	case SC_PROVOKE:	/* ÉvÉçÉ{ÉbÉN/ÉIÅ[ÉgÉoÅ[ÉTÅ[ÉN */
		if (sc_data[type].val2 != 0) { /* ÉIÅ[ÉgÉoÅ[ÉTÅ[ÉNÅiÇPïbÇ≤Ç∆Ç…HPÉ`ÉFÉbÉNÅj */
	...
	case SC_TENSIONRELAX: /* ÉeÉìÉVÉáÉìÉäÉâÉbÉNÉX */
		if (sd) { /* SPÇ™Ç†Ç¡ÇƒÅAHPÇ™ñûÉ^ÉìÇ≈Ç»ÇØÇÍÇŒåpë± */
			if (sd->status.sp > 12 && sd->status.max_hp > sd->status.hp) {
/*				if (sc_data[type].val2 % (sc_data[type].val1+3) == 0) {
					sd->status.sp -= 12;
					clif_updatestatus(sd, SP_SP);
				}*/
				sc_data[type].timer = add_timer(10000 + tick, status_change_timer, bl->id, data); /* É^ÉCÉ}Å[çƒê›íË */
				return 0;
			}
	...
	case SC_WEIGHT50:
	case SC_WEIGHT90:
	case SC_MAGICPOWER:		/* ñÇñ@óÕëùïù */
	case SC_REJECTSWORD:	/* ÉäÉWÉFÉNÉgÉ\Å[Éh */
	...
	case SC_DANCING: //É_ÉìÉXÉXÉLÉãÇÃéûä‘SPè¡îÔ
		{
			...
			if(sd){
				if(sd->status.sp > 0 && (--sc_data[type].val3)>0){
					...
					case BD_DRUMBATTLEFIELD:		/* êÌëæå€ÇÃãøÇ´ 3ïbÇ…SP1 */
					case BD_RINGNIBELUNGEN:			/* ÉjÅ[ÉxÉãÉìÉOÇÃéwó÷ 3ïbÇ…SP1 */
					case BD_SIEGFRIED:				/* ïséÄêgÇÃÉWÅ[ÉNÉtÉäÅ[Éh 3ïbÇ…SP1 */
					...
					case DC_SERVICEFORYOU:			/* ÉTÅ[ÉrÉXÉtÉHÅ[ÉÜÅ[ 5ïbÇ≈SP1 */
						s = 5;
						break;
	...
	case SC_WEDDING: //åãç•óp(åãç•àﬂè÷Ç…Ç»Ç¡Çƒï‡Ç≠ÇÃÇ™íxÇ¢Ç∆Ç©)
		if (sd) {
			time_t timer;
			if (time(&timer) < ((sc_data[type].val2) + 3600)) { //1éûä‘ÇΩÇ¡ÇƒÇ¢Ç»Ç¢ÇÃÇ≈åpë±
				sc_data[type].timer = add_timer(10000 + tick, status_change_timer, bl->id, data); /* É^ÉCÉ}Å[çƒê›íË */
				return 0;
			}
		}
	...
	case SC_NOCHAT: //É`ÉÉÉbÉgã÷é~èÛë‘
		if (sd && battle_config.muting_players) {
			time_t timer;
			if ((++sd->status.manner) && time(&timer) < ((sc_data[type].val2) + 60 * (0 - sd->status.manner))) { //äJénÇ©ÇÁstatus.mannerï™åoÇ¡ÇƒÇ»Ç¢ÇÃÇ≈åpë±
				clif_updatestatus(sd, SP_MANNER);
				sc_data[type].timer = add_timer(60000 + tick, status_change_timer, bl->id, data); /* É^ÉCÉ}Å[çƒê›íË(60ïb) */
				return 0;
			}
		}
		break;
...
int status_readdb(void) {
	...
	while(fgets(line, sizeof(line), fp)) { // fgets reads until maximum one less than size and add '\0' -> so, it's not necessary to add -1
		...
		for(j=0,p=line;j<MAX_LEVEL && p;j++){
			if(sscanf(p,"%d",&k)==0)
				break;
			job_bonus[0][i][j]=k;
			job_bonus[2][i][j]=k; //ó{éqêEÇÃÉ{Å[ÉiÉXÇÕï™Ç©ÇÁÇ»Ç¢ÇÃÇ≈âº
			p=strchr(p,',');
			if(p) p++;
		}
		i++;
	...
	while(fgets(line, sizeof(line), fp)) { // fgets reads until maximum one less than size and add '\0' -> so, it's not necessary to add -1
		...
		refinebonus[i][0] = atoi(split[0]);	// ê∏òBÉ{Å[ÉiÉX
		refinebonus[i][1] = atoi(split[1]);	// âﬂèËê∏òBÉ{Å[ÉiÉX
		refinebonus[i][2] = atoi(split[2]);	// à¿ëSê∏òBå¿äE
