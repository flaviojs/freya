...
//				run_script(sd->petDB->script, 0, sd->bl.id, 0);
	...
	else if (pc_isriding(sd)) {	// ƒyƒRƒyƒRæ‚è‚É‚æ‚é‘¬“x‘‰Á
	...
	if(sd->sc_count){
		...
		if (sd->sc_data[SC_DEFENDER].timer != -1) {
			...
			if (sd->sc_data[SC_DEFENDER].val1 <= 4) // kRO 14/12/04 Patch - No walking speed reduce if level 5 and up. [Aalye]
...
int status_get_size(struct block_list *bl) {
	...
	if (bl->type == BL_MOB)
		...
		if (pc_calc_upper(sd->status.class) == 2)
			return ((pc_isriding(sd)) ? 1 : 0); // Baby Class Peco Rider -> size = 1, else 0
		return ((pc_isriding(sd)) ? 2 : 1); // Peco Rider -> size = 2, else 1
	} else
		return 1;
}
...
*/
		...
		case SC_BLEEDING:
			// Fixed: Bleeding bug (corrections of [BLB] from freya's bug report)
			val4 = tick;
			tick = 10000;
/*			// every 1 vit deducts 1 second
	...
	if (bl->type==BL_PC &&
		((type < SC_SENDMAX && (type != SC_HALLUCINATION || battle_config.display_hallucination)) || type == SC_PRESERVE || type == SC_BATTLEORDERS))
...
int status_change_clear(struct block_list *bl, int type)
{
	...
	for(i = 0; i < MAX_STATUSCHANGE; i++){
		if(sc_data[i].timer == -1 || (type == 0 && (i == SC_EDP || i == SC_MELTDOWN)))
			continue;
	...
	*opt3 = 0;
	...
	if (night_flag == 1 && type == BL_PC && !map[bl->m].flag.indoors &&
	    !map[bl->m].flag.indoors) // && battle_config.night_darkness_level <= 0)
		*opt2 |= STATE_BLIND;
