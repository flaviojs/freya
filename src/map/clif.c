...
/*------------------------------------------
 * @me command by lordalfa, rewritten implementation by Skotlex, added to freya by [akrus]
 *------------------------------------------
*/
int clif_disp_overhead(struct map_session_data *sd, char* mes)
{
	int len_mes = strlen(mes)+1;

	WPACKETW(0) = 0x8d;
	WPACKETW(2) = len_mes + 8;
	WPACKETL(4) = sd->bl.id;
	memcpy(WPACKETP(8), mes, len_mes);
	clif_send(WPACKETW(2), &sd->bl, AREA_CHAT_WOC);

	WPACKETW(0) = 0x8e;
	WPACKETW(2) = len_mes + 4;
	memcpy(WPACKETP(4), mes, len_mes);
...
int clif_damage(struct block_list *src, struct block_list *dst, unsigned int tick, int sdelay, int ddelay, int damage, int div_, int type, int damage2)
{
	...
	if (type != 4 && dst->type == BL_PC && !map[dst->m].flag.gvg && ((struct map_session_data *)dst)->special_state.infinite_endure)
		type = 9;
	if (sc_data) {
		if (type != 4 && (sc_data[SC_ENDURE].timer != -1 || sc_data[SC_CONCENTRATION].timer != -1 || sc_data[SC_BERSERK].timer != -1) && !map[dst->m].flag.gvg)
			type = 9;
	...
	unsigned int tick, int sdelay, int ddelay, int damage, int div_, int skill_id, int skill_lv, int type)
{
	...
	if (type != 5 && dst->type == BL_PC && !map[dst->m].flag.gvg && ((struct map_session_data *)dst)->special_state.infinite_endure)
		type = 9;
	if (sc_data) {
		if (type != 5 && (sc_data[SC_ENDURE].timer != -1 || sc_data[SC_CONCENTRATION].timer != -1 || sc_data[SC_BERSERK].timer != -1) && !map[dst->m].flag.gvg)
			type = 9;
 ...
 *------------------------------------------
 */
void clif_friend_send_online(const int fd, int account_id, int char_id, char flag) {
	WPACKETW( 0) = 0x206;
	WPACKETL( 2) = account_id;
	WPACKETL( 6) = char_id;
	WPACKETB(10) = flag;
...
/*		if (battle_config.night_darkness_level > 0 && !map[sd->bl.m].flag.indoors)
			...
			// night - when changing from indoors to outdoors
			if (!map[sd->bl.m].flag.indoors && (sd->opt2 & STATE_BLIND) != STATE_BLIND)
				sd->opt2 |= STATE_BLIND;
			// changing from outdoors to indoors
			else if (map[sd->bl.m].flag.indoors && (sd->opt2 & STATE_BLIND) == STATE_BLIND)
				sd->opt2 &= ~STATE_BLIND;
		}
	}
