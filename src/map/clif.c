...
void clif_skill_produce_mix_list(struct map_session_data *sd, int trigger)
{
	...
	{
		if( skill_produce_db[ i ].itemlv != trigger ) continue;

		if( skill_can_produce_mix(sd,skill_produce_db[i].nameid) ){
...
void clif_weapon_refine_list(struct map_session_data *sd)
{
	...
	WFIFOW(fd,0)=0x221;
	for(i=c=0;i<MAX_INVENTORY;i++){
		if(sd->status.inventory[i].nameid > 0 && sd->status.inventory[i].identify==1 && sd->inventory_data[i]->refine==1 &&
...
void clif_guild_skillinfo(struct map_session_data *sd, struct guild *g)
{
	...
	for(i = 0; i < MAX_GUILDSKILL; i++) {
		if(g->skill[i].id>0 && guild_check_skill_require(g,g->skill[i].id) && guild_skill_get_max(g->skill[i].id) > 0){ // max=0のものを除外
