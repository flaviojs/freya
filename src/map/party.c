...
//	nullpo_retv(sd); // checked before to call function
	...
	if (!battle_config.invite_request_check) { // Are other requests accepted during a request or not?
		if (tsd->guild_invite > 0 || tsd->trade_partner) { // 相手が取引中かどうか
			clif_party_inviteack(sd, tsd->status.name, 0); // 0: the player is already in other party, 1: invitation was denied, 2: success to invite, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined
			return;
		}
	}
	...
	for(i = 0; i < MAX_PARTY; i++) { // 同アカウント確認
		if (p->member[i].account_id == account_id) {
			clif_party_inviteack(sd, tsd->status.name, 4); // 0: the player is already in other party, 1: invitation was denied, 2: success to invite, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined
			return;
		}
	}
...
//	nullpo_retv(sd); // checked before to call function
	...
	if (flag == 1) { // 承諾 // 0: invitation was denied, 1: accepted
		...
		if (tsd == NULL)
			return;
		clif_party_inviteack(tsd, sd->status.name, 1); // 0: the player is already in other party, 1: invitation was denied, 2: success to invite, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined
	}
...
int party_member_added(int party_id, int account_id, int flag)
{
	...
	if (flag == 1) { // 失敗
		if (sd2 != NULL)
			clif_party_inviteack(sd2, sd->status.name, 0); // 0: the player is already in other party, 1: invitation was denied, 2: success to invite, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined
		return 0;
	}
	...
	if (sd2 != NULL)
		clif_party_inviteack(sd2, sd->status.name, 2); // 0: the player is already in other party, 1: invitation was denied, 2: success to invite, 4: character in the same account already joined, 4: character in the same account already joined, 4: character in the same account already joined
