					...
					if (skill_devotion3(&md->bl,target->id)) {
						...
						for(i = 0; i < 5; i++)
							if (md->dev.val1[i] == target->id) {
								clif_damage(bl, &md->bl, gettick(), 0, 0, damage, 0 , 0, 0);
...
int battle_calc_damage(struct block_list *src, struct block_list *bl, int damage, int div_, int skill_num, int skill_lv, int flag) {
	...
	if (sc_count != NULL && *sc_count > 0) {
				...
				if(rand() % 100 < tsc_data[SC_AUTOGUARD].val2) {
					...
					// different delay depending on skill level [celest]
				  {
					...
					if (tsc_data[SC_AUTOGUARD].val1 <= 5)
						...
						delay = 100;
					if(tsd)
						tsd->canmove_tick = gettick() + delay;
					else if(tmd)
						tmd->canmove_tick = gettick() + delay;
				  }
					//Shrink effect
		...
		if(tsc_data[SC_KYRIE].timer != -1 && damage > 0){	// ÉLÉäÉGÉGÉåÉCÉ\Éì
		...
		if(tsc_data[SC_REJECTSWORD].timer != -1 && damage > 0 && flag&BF_WEAPON &&
			...
			if (rand() % 100 < (15 * tsc_data[SC_REJECTSWORD].val1)) { //îΩéÀämó¶ÇÕ15*Lv
				...
				clif_damage(bl, src, gettick(), 0, 0, damage, 0, 0, 0); // So to display the damage add this [Celest] from Freya' forum
	...
	if (damage > 0) { // damage reductions from GvG or PK mode
		if (map[bl->m].flag.gvg) { //GvG
			if (tmd){ //defenseÇ™Ç†ÇÍÇŒÉ_ÉÅÅ[ÉWÇ™å∏ÇÈÇÁÇµÇ¢ÅH
				struct guild_castle *gc = guild_mapname2gc(map[bl->m].name);
				if (gc) 
...
#define MATK_FIX(a, b) { matk1 = matk1 * (a) / (b); matk2 = matk2 * (a) / (b); }
	...
	if(skill_num > 0) {
		switch(skill_num) {	// äÓñ{É_ÉÅÅ[ÉWåvéZ(ÉXÉLÉãÇ≤Ç∆Ç…èàóù)
		case AL_HEAL:
		case PR_BENEDICTIO:
...
/*
			if( (t_ele!=3 && !battle_check_undead(t_race,t_ele)) || target->type==BL_PC ) //PCÇÕâŒëÆê´Ç≈Ç‡îÚÇ‘ÅHÇªÇ‡ÇªÇ‡É_ÉÅÅ[ÉWéÛÇØÇÈÅH
				blewcount |= 0x10000;
			else
				blewcount = 0;
*/
		...
		case MG_THUNDERSTORM:	// ÉTÉìÉ_Å[ÉXÉgÅ[ÉÄ
			MATK_FIX(80, 100);
			break;
		case MG_FROSTDIVER:	// ÉtÉçÉXÉgÉ_ÉCÉo
			MATK_FIX(100 + skill_lv * 10, 100);
			break;
		case WZ_FROSTNOVA:	// ÉtÉçÉXÉgÉ_ÉCÉo
			MATK_FIX((100 + skill_lv * 10) * 2 / 3, 100);
			break;
		case WZ_FIREPILLAR:	// ÉtÉ@ÉCÉÑÅ[ÉsÉâÅ[
			if(mdef1 < 1000000)
				mdef1 = mdef2 = 0;	// MDEFñ≥éã
			MATK_FIX(1, 5);
			matk1+=50;
			matk2+=50;
			break;
		...
		case WZ_JUPITEL:	// ÉÜÉsÉeÉãÉTÉìÉ_Å[
			break;
		case WZ_VERMILION:	// ÉçÅ[ÉhÉIÉuÉoÅ[É~ÉäÉIÉì
...
//			blewcount|=0x10000;
		...
		case AL_RUWACH:
			MATK_FIX(145, 100);
			break;
		case HW_NAPALMVULCAN:	// ÉiÉpÅ[ÉÄÉrÅ[ÉgÅiï™éUåvéZçûÇ›Åj
			MATK_FIX(70 + skill_lv * 10, 100);
			if(flag>0){
				MATK_FIX(1, flag);
			} else {
...
//	if(mdef1 >= 1000000 && damage > 0)
	...
	if(tsd && status_isimmune(target)) {
		if (battle_config.gtb_pvp_only != 0) { // [MouseJstr]
			...
			damage = 0; // â© ã‡Â≥ÉJÅ[ÉhÅiñÇñ@É_ÉÅÅ[ÉWÇOÅj
	}
