...
static int pet_walk(struct pet_data *pd,unsigned int tick,int data)
{
	...
	if (pd->walkpath.path_half == 0) {
		...
		x = pd->bl.x;
		y = pd->bl.y;
/*		if (map_getcell(pd->bl.m, x, y, CELL_CHKNOPASS)) {
...
void pet_catch_process2(struct map_session_data *sd, int target_id) {
	...
	if (!md || md->bl.type != BL_MOB || md->bl.prev == NULL) {
		sd->catch_target_class = 0; //Reset it, to avoid exploits with custom packets [Proximus]
	...
	if(sd->pet.intimate > 0) {
		...
		if(dist > 12) {
			...
			if (bl_item == NULL || bl_item->type != BL_ITEM ||bl_item->m != pd->bl.m ||
			    (dist=distance(pd->bl.x,pd->bl.y,bl_item->x,bl_item->y))>=5){ // 遠すぎるかアイテムがなくなった
				pet_unlocktarget(pd);
			}
			else if(dist){
...
int pet_heal_timer(int tid, unsigned int tick, int id, int data)
{
	...
	if (battle_config.pet_status_support && sd->status.hp < sd->status.max_hp * pd->skilltype / 100) {
...
int pet_mag_timer(int tid, unsigned int tick, int id, int data)
{
	...
	if (battle_config.pet_status_support && sd->status.hp < sd->status.max_hp * pd->skilltype / 100 && sd->status.sp < sd->status.max_sp * pd->skillduration / 100) {
...
int pet_skillattack_timer(int tid, unsigned int tick, int id, int data)
{
	...
	if (md && battle_config.pet_status_support && rand() % 100 < sd->pet.intimate * pd->skilltimer / 100) {
		switch (pd->skilltype) {
		case SM_PROVOKE:
