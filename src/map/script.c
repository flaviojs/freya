 ...
 *------------------------------------------
 */
static int check_arrayscope( char prefix, const char *func )
{
	if( prefix != '$' && prefix != '@' && prefix != '\'' )
	{
		printf( "%s: illegal scope !\n", func );
		return 0;
	}
	else
	{
 ...
 *------------------------------------------
 */
#define IS_CHAR_VAR( prefix ) ( ( prefix ) != '$' && ( prefix ) != '\'' )
...
struct script_function buildin_func[] = {
	...
	{buildin_arrayselect,"arrayselect","i*"},
...
int buildin_setarray(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_setarray" ) ) return 0;
	if( IS_CHAR_VAR( name[ 0 ] ) ) sd = script_rid2sd( st );
...
int buildin_cleararray(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_cleararray" ) ) return 0;
	if( IS_CHAR_VAR( name[ 0 ] ) ) sd = script_rid2sd( st );
...
int buildin_copyarray(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_copyarray" ) ||
		!check_arrayscope( name2[ 0 ], "buildin_copyarray" ) ) return 0;
	...
	if( IS_CHAR_VAR( name[ 0 ] ) || IS_CHAR_VAR( name2[ 0 ] ) ) sd = script_rid2sd( st );
...
int buildin_getarraysize(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_getarraysize" ) ) return 0;
...
int buildin_deletearray(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_deletearray" ) ) return 0;
	if( IS_CHAR_VAR( name[ 0 ] ) ) sd = script_rid2sd( st );
...
int buildin_getwaitingpcid(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_getwaitingpcid" ) ) return 0;
	if( IS_CHAR_VAR( name[ 0 ] ) ) sd = script_rid2sd( st );
...
// packed index utility
#define ARRAYIDX_GETIDX( packed )	( ( (packed) >> 24 ) & 0xff )
#define ARRAYIDX_GETDATA( packed )	( (packed) & 0x00ffffff )
#define ARRAYIDX_PACK( idx, data )	( ( ( (idx) << 24 ) & 0xff000000 ) | ( (data) & 0x00ffffff ) )
...
// 書き込み用csvのファイル名をチェック
static int check_csvfilename_write( const char *file, const char *func )
{
	for( ; *file; ++file )
	{
		if( !isalnum( *file & 0xFF ) ) {
...
int buildin_csvreadarray(struct script_state *st)
{
	...
	// clear array
	if( !check_arrayscope( name[ 0 ], "buildin_csvreadarray" ) ) return 0;
	if( IS_CHAR_VAR( name[ 0 ] ) )
	{
		sd = script_rid2sd( st );
		if( !sd ) return 0;
	}
...
int buildin_csvwritearray(struct script_state *st)
{
	...
	if( !check_arrayscope( name[ 0 ], "buildin_csvwritearray" ) ) return 0;
	if( !check_csvfilename_write( file, "buildin_csvwritearray" ) ) return 0;
...
int buildin_csvinsert(struct script_state *st) {
	...
	if( !check_csvfilename_write( file, "buildin_csvinsert" ) ) return 0;
...
int buildin_csvdelete(struct script_state *st) {
	...
	if( !check_csvfilename_write( file, "buildin_csvdelete" ) ) return 0;
...
int buildin_csvsort(struct script_state *st) {
	...
	if( !check_csvfilename_write( file, "buildin_csvsort" ) ) return 0;
...
int buildin_csvflush(struct script_state *st) {
	...
	if( !check_csvfilename_write( file, "buildin_csvflush" ) ) return 0;
