...
#define SCRIPT_HASH_SIZE 16	/* optimal script hash size */
...
static char pos[11][100] = {"","�,"�,"E�,"[u","C","ANZT[1","ANZT[2","2","3","ĂȂ"};
...
struct {
	...
	{buildin_areamonsteragro,"areamonsteragro","siiiisii*"}, // as areamonster, but all monster are agressiv
	...
	{buildin_getmapgms2,"getmapgms2","si"}, // as getmapusers2, return number of ALIVE GMs on a map (with minimum level of concerned GMs)
 ...
 * XNvgobt@Ƀx/ϐ/֐� *------------------------------------------
 */
// ő�6M܂�static void add_scriptl(int l) {
	...
	case C_POS:
		...
		break;
	case C_NOP:
		// x̉\��ackpatchpf[^ߍ�		add_scriptc(C_NAME);
	...
	case C_INT:
		...
		break;
	default:
		// ¤̗prƊm肵Ă�Ő⻂̂܂�		add_scriptc(C_NAME);
		add_scriptb(l);
		add_scriptb(l >> 8);
		add_scriptb(l >> 16);
		break;
...
unsigned char* parse_simpleexpr(unsigned char *p)
{
	...
	if(*p=='('){
		...
		p2=(char*)skip_word(p);
		c = *p2; // O�dd_str�		*p2 = 0; // O�dd_str�		l = add_str((char*)p);

		parse_cmd = l; // warn_*_mismatch_paramnum̂߂ɕKv
		if (l == search_str("if")) // warn_cmd_no_commâ߂ɕKv
			parse_cmd_if++;
...
unsigned char* parse_line(unsigned char *p)
{
	...
	parse_cmd_if = 0; // warn_cmd_no_commâ߂ɕKv
...
/* load contant database into memory */
void read_constdb(void)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		if((line[0] == '/' && line[1] == '/') || line[0] == '\0' || line[0] == '\n' || line[0] == '\r')
			continue;
		type = 0;
		if(sscanf(line, "%[A-Za-z0-9_],%d,%d", name, &val, &type) >= 2 || sscanf(line, "%[A-Za-z0-9_] %d %d", name, &val, &type) >= 2)
		{
			n = add_str(name);
			if (type == 0)
...
unsigned char* parse_script(unsigned char *src, int line)
{
	...
	CALLOC(script_buf, unsigned char, SCRIPT_BLOCK_SIZE);
	script_pos = 0;
	script_size = SCRIPT_BLOCK_SIZE;
	...
	for(p++;p && *p && *p!='}';){
		...
		if(*tmpp==':'){
			...
			*skip_word(p)=0;
			...
			strdb_insert(scriptlabel_db,p,script_pos);	// Oplabel dbo^
			*skip_word(p)=c;
			p=tmpp+1;
			continue;
		}
...
int buildin_callfunc(struct script_state *st)
{
	...
	if ((scr = strdb_search(script_get_userfunc_db(), str))) {
		...
		push_val(st->stack,C_INT,j);				// ̐�bV
		push_val(st->stack,C_INT,st->defsp);	// ݂̊X^bN|C^�bV
		push_val(st->stack,C_INT,(int)st->script);	// ݂̃XNvg�bV
		push_val(st->stack,C_RETINFO,st->pos);		// ݂̃XNvgʒu�bV
...
int buildin_return(struct script_state *st)
{
	if(st->end>st->start+2){	// ߂�L�		push_copy(st->stack,st->start+2);
	}
	st->state=RETFUNC;
...
//		buf[0]=0;
		...
		st->state=END;
	} else {	// goto�		// ragemu݊̂�		pc_setreg(sd,add_str("l15"),sd->npc_menu);
...
//	char prefix=*name;
	...
	if(sd->state.menu_or_input){
		sd->state.menu_or_input=0;
		if (name[strlen(name)-1] == '$') { // postfix
			// �			if(st->end>st->start+2){ // 1�				set_reg(sd, num, name, (void*)sd->npc_str); // npc_str = 255 + NULL
			}else{
...
//				buildin_close(st); // ** close
			...
			// l
			if(st->end>st->start+2){ // 1�				set_reg(sd,num,name,(void*)sd->npc_amount);
			} else {
				// ragemu݊̂�				pc_setreg(sd,add_str("l14"),sd->npc_amount);
			}
		}
	} else {
		st->state = RERUNLINE;
...
/*==========================================
 * Count an specific item from cart [Proximus]
 *------------------------------------------
 */
int buildin_countitemcart(struct script_state *st)
{
...
int buildin_getitem(struct script_state *st)
{
	...
	if(nameid > 0) {
		...
		if (st->end>st->start + 5) //ACe�肵IDɓn
			sd=map_id2sd(conv_num(st, &(st->stack->stack_data[st->start+5])));
		if (sd == NULL) //ACe�肪ȂB炨A�			return 0;
...
int buildin_getitem2(struct script_state *st)
{
	...
	if( st->end>st->start+11 ) //ACe�肵IDɓn
...
/*====================================
 * Return number of ALIVE GMs on a map
 *------------------------------------
 */
...
/*===================================
 * Return number of DEAD GMs on a map
 *-----------------------------------
 */
...
int buildin_sc_start(struct script_state *st)
{
	...
	if (st->end>st->start + 5) //w肵L�Ԉُ��		bl = map_id2bl(conv_num(st, &(st->stack->stack_data[st->start+5])));
	else
		bl = map_id2bl(st->rid);
	if (bl) {
...
int buildin_sc_start2(struct script_state *st)
{
	...
	if (st->end>st->start + 6) //w肵L�Ԉُ��		bl = map_id2bl(conv_num(st,& (st->stack->stack_data[st->start+6])));
	else
		bl = map_id2bl(st->rid);
	if (bl) {
		if (bl->type == BL_PC && ((struct map_session_data *)bl)->state.potionpitcher_flag)
...
int buildin_getscrate(struct script_state *st)
{
	...
	if (st->end>st->start + 4) //w肵L̑ϐ�Z�		bl = map_id2bl(conv_num(st, &(st->stack->stack_data[st->start+6])));
	else
		bl = map_id2bl(st->rid);
...
int buildin_globalmes(struct script_state *st)
{
	...
	mes = conv_str(st, &(st->stack->stack_data[st->start + 2])); // bZ[W̎擾
	if (mes == NULL)
		return 0;

	if (st->end > st->start + 3){ // NPC̎擾(123#456)
...
int buildin_warpwaitingpc(struct script_state *st)
{
	...
	for(i=0;i<n;i++){
		struct map_session_data *sd=cd->usersd[0];	// Xg擪�CXɁB
...
int buildin_getequipcardcnt(struct script_state *st)
{
	...
	sd=script_rid2sd(st);
	i=pc_checkequip(sd,equip[num-1]);
	if(sd->status.inventory[i].card[0] == 0x00ff){ // �J[hȂ
		push_val(st->stack,C_INT,0);
		return 0;
	}
	do {
...
int buildin_successremovecards(struct script_state *st)
{
	...
	sd=script_rid2sd(st);
	i=pc_checkequip(sd,equip[num-1]);
	if(sd->status.inventory[i].card[0]==0x00ff){ // �Ȃ
		return 0;
	}
	do{
		if ((sd->status.inventory[i].card[c-1] > 4000 &&
			...
			if ((flag = pc_additem(sd, &item_tmp, 1))) { // ĂȂȂ�bv
				clif_additem(sd, 0, 0, flag);
	...
	if(cardflag == 1){	// J[h�菜ACe
		...
		item_tmp.card[0]=0,item_tmp.card[1]=0,item_tmp.card[2]=0,item_tmp.card[3]=0;
		pc_delitem(sd,i,1,0);
		if ((flag = pc_additem(sd, &item_tmp, 1))) { // �ȂȂ�bv
			clif_additem(sd, 0, 0, flag);
			map_addflooritem(&item_tmp, 1, sd->bl.m, sd->bl.x, sd->bl.y, NULL, NULL, NULL, sd->bl.id, 0);
		}
		clif_misceffect(&sd->bl, 3);
...
int buildin_failedremovecards(struct script_state *st)
{
	...
	do {
		if ((sd->status.inventory[i].card[c-1] > 4000 &&
			...
			if(typefail == 2){ // �ݑȂ�J[h͎󂯎��				item_tmp.id=0,item_tmp.nameid=sd->status.inventory[i].card[c-1];
				item_tmp.equip = 0, item_tmp.identify = 1, item_tmp.refine = 0;
				item_tmp.attribute=0;
				item_tmp.card[0]=0,item_tmp.card[1]=0,item_tmp.card[2]=0,item_tmp.card[3]=0;
...
//		st->stack->sp=0;
	...
	if(st->state==RETFUNC){
		...
		pop_stack(st->stack,st->defsp,start_sp);	// AɎזȃX^bN폜
		if(st->defsp<4 || st->stack->stack_data[st->defsp-1].type!=C_RETINFO){
		...
		st->pos=conv_num(st,& (st->stack->stack_data[st->defsp-1]));			// XNvgʒu̕
		st->script=(char*)conv_num(st,& (st->stack->stack_data[st->defsp-2]));	// XNvg𕜌
		st->defsp=conv_num(st,& (st->stack->stack_data[st->defsp-3]));			// X^bN|C^𕜌

		pop_stack(st->stack,olddefsp-4-i,olddefsp);		// v�ȂBX^bN(ƕApf[^)폜
