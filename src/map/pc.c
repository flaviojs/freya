...
//	map_addnickdb(sd);
	...
	if (night_flag == 1 && !map[sd->bl.m].flag.indoors) {
		clif_wis_message(sd->fd, wisp_server_name, msg_txt(500), strlen(msg_txt(500)) + 1); // Actually, it's the night...
/*		if (battle_config.night_darkness_level > 0)
			clif_specialeffect(&sd->bl, 474 + battle_config.night_darkness_level, 0); // flag: 0: player see in the area (normal), 1: only player see only by player, 2: all players in a map that see only their (not see others), 3: all players that see only their (not see others)
		else*/
			//clif_specialeffect(&sd->bl, 483, 0); // default darkness level // flag: 0: player see in the area (normal), 1: only player see only by player, 2: all players in a map that see only their (not see others), 3: all players that see only their (not see others)
			sd->opt2 |= STATE_BLIND;
	}
...
int pc_setpos(struct map_session_data *sd, char *mapname_org, int x, int y, int clrtype) {
	...
	if ((sd->state.bot_flag % 2) == 1) { // 0: no bot action done, 1: far fake player sended, 2: far fake player deleted, 3: hiden fake player sended, 4: hiden fake player deleted, 5: like 1, 6: like 2
		// remove fake people from screen, because with flying wings, you can see them
		// take fake mob out of screen
		clif_clearchar_id(server_fake_mob_id, 0, sd->fd);
		// take fake player out of screen
		clif_clearchar_id(server_char_id, 0, sd->fd);
	...
	// Player has een invited to be a friend
...
int pc_jobchange(struct map_session_data *sd, int job, int upper)
{
	...
	if ((job >= 7 && job <= 21) || (job >= 4008 && job <= 4022) || (job >= 4030 && job <= 4044)) {
		if ((s_class.job > 0 && s_class.job < 7) || (s_class.job > 4001 && s_class.job < 4008) || (s_class.job > 4023 && s_class.job < 4030))
...
// Simplified reg removal, do we need so many unused things? ;)
	...
	if (sd->global_reg_num < GLOBAL_REG_NUM) {
		...
		sd->global_reg[sd->global_reg_num].str[32] = '\0';
...
// -- moonsoul (if conditions below altered to disallow natural healing if under berserk status)
	...
	if (pc_checkskill(sd, MO_SPIRITSRECOVERY) > 0 && !pc_ishiding(sd) &&
