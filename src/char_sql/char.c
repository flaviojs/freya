...
//	printf("- Save char data to MySQL!\n");
	sql_request("UPDATE `%s` SET `class`='%d',`base_level`='%d',`job_level`='%d',"
	            ...
	            "`last_map`='%s',`last_x`='%d',`last_y`='%d',`save_map`='%s',`save_x`='%d',`save_y`='%d',`partner_id`='%d' WHERE `char_id` = '%d' LIMIT 1",
...
int mmo_char_fromsql(int char_id) {
	...
	sql_request("SELECT `str`, `value` FROM `%s` WHERE `char_id`='%d'", global_reg_value, char_id);
	i = 0;
	while(sql_get_row() && i < GLOBAL_REG_NUM) {
		if (sql_get_string(0) != NULL && sql_get_string(0)[0] && sql_get_integer(1) != 0) {
			...
			for (j = 0; j < i; j++)
				if (strcmp(reg[j].str, sql_get_string(0)) == 0)
					break;
			// if not found
			if (j == i) {
...
//				printf("reg #%d: %s=%d\n", i, reg[i].str, reg[i].value);
	...
	if (i > 0) {
		global_reg_db[char_num].global_reg_num = i;
		MALLOC(global_reg_db[char_num].global_reg, struct global_reg, i);
		memcpy(global_reg_db[char_num].global_reg, reg, sizeof(struct global_reg) * i);
	}
