...
#endif /* TXT_ONLY */
#endif /* SVN_REVISION */
	puts(CL_GREEN "Website/Forum" CL_RESET ": \thttp://www.ro-freya.net/");
...
void display_title(void) {
	...
	printf(CL_DARK_WHITE CL_BG_BLUE "          (" CL_BOLD "          (c)2004-2005 Free Yor team presents :          " CL_DARK_WHITE CL_BG_BLUE ")" CL_CLL CL_RESET "\n");
...
int main(int argc, char **argv) {
	...
	runflag = 1;
	while(runflag) {
		gettick_nocache(); // before to check timer, reinit tick
		next = do_timer();
		do_sendrecv(next);
		gettick_nocache(); // before to parse, reinit tick
		do_parsepacket();
	}
